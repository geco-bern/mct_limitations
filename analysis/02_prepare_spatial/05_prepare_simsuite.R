...
dir_climate <- Sys.getenv(
  "MCT_SOFUN_OUTPUT",
  path.expand("~/sofun/output_nc_global_sofun")
)
dir_fapar <- "~/sofun/input_sj02zroot_sofun/sitedata/fapar/"
path_root_profiles <- Sys.getenv(
  "MCT_ROOT_PROFILES",
  path.expand("~/data/rootingdepth/root_profiles_schenkjackson02/data/root_profiles_D50D95.csv")
)

## get meta info of sites (lon, lat)
df_sites <- read_csv(path_root_profiles) %>%
