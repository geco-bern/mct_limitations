...
```{r}
path_siteinfo <- "data/siteinfo_mct_fluxnet2015.csv"
...
settings_calib <- list(
  ...
  metric           = "rmse",
  dir_results      = "data/calib_results/"
)
```
 
Get calibration target data.
```{r warning=FALSE, messge=FALSE}
filn <- "./data/ddf_obs_calib.rds"
if (!file.exists(filn)){
  ...
  saveRDS(ddf_obs_calib, file = filn)
} else {
  ddf_obs_calib <- readRDS(filn)
}
```
...
```{r}
filn <- "./data/out_sitebysite.rds"
if (file.exists(filn)){
  out_sitebysite <- readRDS(filn)
} else {
  ...
  saveRDS(out_sitebysite, file = filn)
}
```
...
df_params <- extract_params_oob(param = "vpdstress_par_a", path = "data/calib_results/sitebysite_MCT", sitenames = df_metrics$sitename)
