...
Load ALEXI-ET and WATCH-WFDEI precipitation data for each site. This is prepared by `analysis/02_prepare_spatial/03_get_sj02_data.R`. This loads an object called `df_alexi`.
```{r}
df_alexi <- readRDS("data/df_alexi_sj02.rds")
```
...
saveRDS(df_alexi_repaired, file = "data/df_alexi_repaired_sj02.rds")
```
...
saveRDS(df_alexi_snow, file = "data/df_alexi_snow_sj02.rds")
```
...
filn <- "data/df_mct_sj02_alexi_reOLD.rds"
...
if (!file.exists(filn)){
  ...
  saveRDS(df_mct_sj02_alexi, file = filn)
  
} else {
  df_mct_sj02_alexi <- readRDS(filn)
}
```
...
```{r}
df_whc <- readRDS("data/df_whc_sj02_reOLD.rds")
df_alpine <- df_modobs %>%
...
```{r}
# ddf_mct_simsuite <- readRDS("data/ddf_mct_simsuite.rds") xxx ??? don't know what this is for
df_mct_sj02_alexi %>% 
...
saveRDS(df_whc, file = "data/df_whc_sj02_reOLD.rds")
```
...
saveRDS(df_zroot, file = "data/df_zroot_sj02_reOLD.rds")
```
...
saveRDS(df_modobs, file = "data/df_modobs_reOLD.rds")
```
...
saveRDS(df_modobs, file = "data/df_modobs_sj02_reOLD.rds")
```
...
saveRDS(df_wwf, file = "./data/df_wwf_sj02.rds")
```
...
saveRDS(df_modobs, file = "data/df_modobs_sj02.rds")
```
...
saveRDS(df_modobs_reOLD, file = "data/df_modobs_reOLD.rds")
```
