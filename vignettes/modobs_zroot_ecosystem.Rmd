...
#df_obs2 <- read_csv("~/data/RPGE_shersing/data/root_profiles_D50D95.csv")
df_obs <- read_csv("~/data/rootingdepth/root_profiles_schenkjackson02/data/root_profiles_D50D95.csv") %>% 
...
```{r}
df_20y_bl_v3 <- readRDS("./data/df_20y_bl_v3.rds")
df_20y_bl_v3 %>% 
...
```{r echo = FALSE}
kgclimate_lores <- raster("~/data/koeppengeiger/Beck_KG_V1/Beck_KG_V1_present_0p5.tif")
kgclimate_hires <- raster("~/data/koeppengeiger/Beck_KG_V1/Beck_KG_V1_present_0p0083.tif")
...
rasta <- raster("~/data/biomes/NCRS/global_biomes_geotiff/biomes.tif")
plot(rasta)
...
# df_20y_sr_v2 <- readRDS("./data/df_20y_sr_v2.rds")
# df_20y_bl_v2 <- readRDS("./data/df_20y_bl_v2.rds")
df_20y_sr_v3 <- readRDS("./data/df_20y_sr_v3.rds")
df_20y_bl_v3 <- readRDS("./data/df_20y_bl_v3.rds")
...
```{r eval=FALSE}
df_anders <- nc_to_df(
  "~/data/landclass_ahlstroem/semiarid_shrub_savannah.nc",
  ...
  left_join(
    nc_to_df(
      "~/data/landclass_ahlstroem/forest_extratropical.nc",
  ...
  left_join(
    nc_to_df(
      "~/data/landclass_ahlstroem/forest_tropical.nc",
  ...
  left_join(
    nc_to_df(
      "~/data/landclass_ahlstroem/grassland_crops.nc",
  ...
  left_join(
    nc_to_df(
      "~/data/landclass_ahlstroem/sparse_veg.nc",
      varnam = "sparsely veg"),
    by = c("lon", "lat")
  ) %>%
  mutate( myvar = ifelse(is.nan(myvar), NA, myvar)) %>%
  ...
  left_join(
    nc_to_df(
      "~/data/landclass_ahlstroem/tundra_cool_shrub.nc",
...
saveRDS(df_anders, file = "data/df_anders.rds")
