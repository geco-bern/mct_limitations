...
```{r echo=FALSE}
df_sr <- nc_to_df("~/data/soil/hwsd/hwsd_wieder/data_calculated_halfdeg/T_sr_WHC.nc", "whc") %>% 
  rename(whc_t = myvar) %>% 
  left_join(
    nc_to_df("~/data/soil/hwsd/hwsd_wieder/data_calculated_halfdeg/S_sr_WHC.nc", "whc") %>% 
      rename(whc_s = myvar),
    by = c("lon", "lat")
  ) %>% 
  drop_na()

df_bl <- nc_to_df("~/data/soil/hwsd/hwsd_wieder/data_calculated_halfdeg/T_bl_WHC.nc", "whc") %>% 
  rename(whc_t = myvar) %>% 
  left_join(
    nc_to_df("~/data/soil/hwsd/hwsd_wieder/data_calculated_halfdeg/S_bl_WHC.nc", "whc") %>% 
      rename(whc_s = myvar),
    by = c("lon", "lat")
  ) %>% 
  drop_na()
...
saveRDS(df_40y_sr_v1, file = "./data/df_40y_sr_v1.rds")

df_20y_sr_v1 <- df_sr_v1 %>% 
  rowwise() %>% 
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_20y, whc_t, whc_s))
saveRDS(df_20y_sr_v1, file = "./data/df_20y_sr_v1.rds")

df_40y_bl_v1 <- df_bl_v1 %>% 
  rowwise() %>% 
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_40y, whc_t, whc_s))
saveRDS(df_40y_bl_v1, file = "./data/df_40y_bl_v1.rds")

df_20y_bl_v1 <- df_bl_v1 %>% 
  rowwise() %>% 
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_20y, whc_t, whc_s))
saveRDS(df_20y_bl_v1, file = "./data/df_20y_bl_v1.rds")
...
df_40y_sr_v3 <- df_sr_v3 %>% 
  ...
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_40y, whc_t, whc_s))
saveRDS(df_40y_sr_v3, file = "./data/df_40y_sr_v3.rds")

df_20y_sr_v3 <- df_sr_v3 %>% 
  rowwise() %>% 
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_20y, whc_t, whc_s))
saveRDS(df_20y_sr_v3, file = "./data/df_20y_sr_v3.rds")

df_40y_bl_v3 <- df_bl_v3 %>% 
  rowwise() %>% 
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_40y, whc_t, whc_s))
saveRDS(df_40y_bl_v3, file = "./data/df_40y_bl_v3.rds")

df_20y_bl_v3 <- df_bl_v3 %>% 
  rowwise() %>% 
  drop_na() %>% 
  mutate(zroot = calc_zroot(whc_mct_20y, whc_t, whc_s))
saveRDS(df_20y_bl_v3, file = "./data/df_20y_bl_v3.rds")
```
...
write_nc2(
  ...
  varnam = "zroot", 
  filnam_template = "~/sofun/output_nc_global_sofun/s1_fapar3g_v3_global.fland.nc", 
