...
Let's try this out for the FLUXNET one site for which whe have these variables. Read FLUXNET meteo data from FLUXNET stations (and years), written by `analysis/08_site_analysis/03_evaluate_fluxnet.R`.
```{r}
## meteo data 
ddf_meteo <- readRDS("./data/ddf_meteo.rds")

## re-read ET data from FLUXNET and don't remove (filter) any data
filn <- "data/ddf_eval_all.rds"
if (!file.exists(filn)){
  ddf_eval <- ingest(
    siteinfo = siteinfo,
    source    = "fluxnet", 
...
Read ALEXI ET and WATCH-WFDEI data at FLUXNET sites, written by `analysis/02_prepare_spatial/02_get_fluxnet_data.R`, and add SIF data.
```{r}
df_alexi <- readRDS("data/df_alexi.rds")
df_sif <- readRDS("data/df_sif.rds")
