...
Invoke all at once. This takes several days. Chunks of data are written to `./data/df_plantwhc_mct<i>.rds`, where `i` runs from 1 to 1000.
```{r eval=FALSE}
## The maintained global preparation is in analysis/02_prepare_spatial/04_prepare_global_grid.R
...
data_path <- "./data/v3/"
files <- list.files(data_path, pattern = "^df_plantwhc_mct.*[.]rds$")
df_mct <- purrr::map_dfr( files, ~read_onefile(paste0(data_path, .))) %>% 
  unnest(out_ilon_ilat)
saveRDS(df_mct, file = "data/df_mct_v3.rds")
```
...
## v3 data
df_mct_v3 <- readRDS("data/df_mct_v3.rds")
...
## save as NetCDF
dirnam <- "~/sofun/output_nc_global_sofun/"
write_nc2(
  var = grid_mct_20y_v3, 
  varnam = "whc_mct_20y", 
