...
filnam <- paste0("data/", zid, "_df_sr.rds")
if(!exists("df_srXXX")){
  if (!file.exists(filnam)){
    df_sr <- df %>% calc_soilparams(method = "saxtonrawls")
    saveRDS(df_sr, file = filnam)
  } else {
    df_sr <- readRDS(filnam)
  }
}
```
...
```{r warning = FALSE, message = FALSE}
filnam <- paste0("data/", zid, "_df_bl.rds")
if(!exists("df_blXXX")){
  if (!file.exists(filnam)){
    df_bl <- df %>% calc_soilparams(method = "balland")
    saveRDS(df_bl, file = paste0("data/", zid, "_df_bl.rds"))
  } else {
    df_bl <- readRDS(filnam)
  }
}
```
...
```{r}
if (!exists("df_bl")) df_bl <- readRDS(paste0("data/", zid, "_df_bl.rds"))
if (!exists("df_sr")) df_sr <- readRDS(paste0("data/", zid, "_df_sr.rds"))
df_tmp <- df_sr %>% 
  select(whc_sr = whc) %>% 
...
system(paste0("cdo remapbil,data-raw/grid/gridfile_halfdeg.txt ~/data/soil/hwsd/hwsd_wieder/data_calculated/", zid, "_sr_WHC.nc ~/data/soil/hwsd/hwsd_wieder/data_calculated_halfdeg/", zid, "_sr_WHC.nc"))

system(paste0("cdo remapbil,data-raw/grid/gridfile_halfdeg.txt ~/data/soil/hwsd/hwsd_wieder/data_calculated/", zid, "_bl_WHC.nc ~/data/soil/hwsd/hwsd_wieder/data_calculated_halfdeg/", zid, "_bl_WHC.nc"))
