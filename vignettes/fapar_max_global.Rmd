...
```{r}
df <- nc_to_df(
  obj = "~/data/modis_monthly-evi/zmaw_data/halfdeg/modis_vegetation__LPDAAC__v5__0.5deg_MEAN.nc",
  varnam = "evi") %>% 
  rename( evi = myvar ) %>% 
  left_join( 
    nc_to_df(
      obj = "~/data/fAPAR/fAPAR3g_v2/fAPAR3g_v2_1982_2016_FILLED_MEAN.nc",
      varnam = "FAPAR_FILLED") %>% rename( fapar3g = myvar ),
    by = c("lon", "lat") ) %>% 
  left_join(
    nc_to_df(
      obj = "~/data/landmasks/gicew_halfdeg.cdf",
      varnam = "GICEW"
    ) %>% rename( gicew = myvar ),
    by = c("lon", "lat")
  ) %>% 
...
```{r}
df_anders <- readRDS("data/df_anders.rds")
df <- df %>% 
