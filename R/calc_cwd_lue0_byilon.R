calc_cwd_lue0_byilon <- function(ilon, drop_data = TRUE,
  ...
  path <- climate_output_path(
    file.path(dirn, paste0("df_cwd_lue0_", ilon, ".rds")),
    config
  )
  ...
  if (!file.exists(path)){
    ...
    df <- readRDS(path.expand(sw_path))
    df_sw <- df %>%
    ...
    sif_pk_path <- climate_output_path(
      paste0(
        ...
        "GOME_PK_dcSIF_005deg_8day__ilon_", ilon, ".rds"
      ),
      config
    )
    df <- readRDS(path.expand(sif_pk_path)) # loads 'df'
    df_pk <- df %>%
      mutate(lon = round(lon, digits = 3), lat = round(lat, digits = 3)) %>%
      rename(data_pk = data)
    ...
    sif_jj_path <- climate_output_path(
      paste0(
        "~/data/gome_2_sif_downscaled/data_tidy/",
        "GOME_JJ_dcSIF_005deg_8day__ilon_", ilon, ".rds"
      ),
      config
    )
    df <- readRDS(path.expand(sif_jj_path)) # loads 'df'
